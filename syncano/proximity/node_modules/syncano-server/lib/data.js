...
var Data = function (_QueryBuilder) {
  ...
  _createClass(Data, [{
    ...
    value: function url(id) {
      ...
      var url = this._getInstanceURL(instanceName) + '/classes/' + className + '/objects/' + (id ? id + '/' : '');
    ...
    value: function _batchBodyBuilder(body) {
      var apiVersion = this.instance.apiVersion;

      var path = '/' + apiVersion + this.url().split(apiVersion)[1].split('?')[0];
    ...
    value: function list() {
      ...
      var pageSize = query.page_size || 0;
      ...
      return new Promise(function (resolve, reject) {
        ...
        function loadNextPage(response) {
          ...
          if (hasNextPageMeta && hasNotEnoughResults) {
            var next = response.next.replace(/\?.*/, '');
            var nextParams = _querystring2.default.parse(response.next.replace(/.*\?/, ''));
            var q = _querystring2.default.stringify(_extends({}, query, nextParams));
    ...
    key: '_replaceCustomTypesWithValue',
    value: function _replaceCustomTypesWithValue(items) {
      var _this3 = this;

      if (Array.isArray(items)) {
        return items.map(function (item) {
          return Object.keys(item).reduce(function (all, key) {
            return _extends({}, all, _defineProperty({}, key, _this3._replaceCustomType(key, item)));
          }, {});
        });
      }

      return Object.keys(items).reduce(function (all, key) {
        return _extends({}, all, _defineProperty({}, key, _this3._replaceCustomType(key, items)));
      }, {});
    }
  }, {
    key: '_replaceCustomType',
    value: function _replaceCustomType(key, item) {
      var value = item[key];
    ...
    value: function _mapFields(items) {
      ...
      if (Array.isArray(items)) {
        return items.map(function (item) {
          return _this4._mapFieldsForSingleItem(item, fields);
        });
      }
    ...
    value: function firstOrFail() {
      ...
      return new Promise(function (resolve, reject) {
        _this5.first().then(function (object) {
    ...
    value: function updateOrCreate(attributes) {
      ...
      return this.where(query).firstOrFail().then(function (res) {
        return _this7.update(res.id, values);
      }).catch(function () {
        return _this7.create((0, _lodash6.default)(attributes, values));
      });
    }
  }, {
    key: '_toWhereArray',
    ...
    value: function findOrFail(ids) {
      ...
      return new Promise(function (resolve, reject) {
        _this8.find(ids).then(function (response) {
    ...
    value: function where(column, operator, value) {
      ...
      if (Array.isArray(column)) {
        column.map(function (_ref2) {
          ...
          return _this9.where(itemColumn, itemOperator, itemValue);
        });
      ...
      var whereOperator = value !== undefined ? '_' + operator : '_eq';
    ...
    value: function _batch(body, headers) {
      ...
      var requests = this._chunk(body, MAX_BATCH_SIZE).map(function (chunk) {
        return function () {
          var fetchObject = _this10._batchFetchObject(chunk);

          return _this10.fetch(fetchObject.url, fetchObject, headers);
        };
      });
    ...
    value: function create(body) {
      ...
      if (body instanceof _formData2.default) {
        ...
        return this._batch(body, headers).then(this._replaceCustomTypesWithValue.bind(this)).then(this._mapFields.bind(this));
      } else {
        fetchObject.body = JSON.stringify(body);
      }

      return this.fetch(fetchObject.url, fetchObject, headers).then(this._replaceCustomTypesWithValue.bind(this)).then(this._mapFields.bind(this));
    }
    ...
    value: function update(id, body) {
      ...
      if (isQueryUpdate) {
        return this.list().then(function (items) {
          ...
          return _this11._batch(ids).then(_this11._replaceCustomTypesWithValue.bind(_this11)).then(_this11._mapFields.bind(_this11));
        });
      }
      ...
      if (body instanceof _formData2.default) {
        ...
        return this._batch(id).then(this._replaceCustomTypesWithValue.bind(this)).then(this._mapFields.bind(this));
      }
